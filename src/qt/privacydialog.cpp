                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBLX ought to be enough for anybody." - Bill Gates, 2017
    ui->zBLXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBLXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBLX->setEnabled(false);
        ui->pushButtonMintzBLX->setToolTip(tr("zBLX is currently disabled due to maintenance."));

        ui->pushButtonSpendzBLX->setEnabled(false);
        ui->pushButtonSpendzBLX->setToolTip(tr("zBLX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBLXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBLX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBLX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBLX in ") +
...
void PrivacyDialog::on_pushButtonSpendzBLX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBLX
        sendzBLX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBLX
    sendzBLX();
}

void PrivacyDialog::on_pushButtonZblxControl_clicked()
{
    ZblxControlDialog* zBLXControl = new ZblxControlDialog(this);
    zBLXControl->setModel(walletModel);
    zBLXControl->exec();
}

void PrivacyDialog::setZblxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBLXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBLX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BLX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBLXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBLX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BLX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBLX</b>";
    ...
    // use mints from zBLX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZblxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZblxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zblx selector in case it was used
    ZblxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBLX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BLX, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBLX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBLX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBLX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBLX "));
    ui->labelzBLXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBLXSyncStatus->setVisible(fShow);
}
