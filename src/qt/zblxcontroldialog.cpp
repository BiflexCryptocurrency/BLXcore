...
#include "ui_zblxcontroldialog.h"
...
std::list<std::string> ZblxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZblxControlDialog::listMints;

ZblxControlDialog::ZblxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZblxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZblxControlDialog::~ZblxControlDialog()
{
    delete ui;
}

void ZblxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZblxControlDialog::updateList()
{
...
void ZblxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZblxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZblx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZblxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZblxControlDialog::GetSelectedMints()
{
...
void ZblxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
