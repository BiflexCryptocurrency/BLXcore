...
*Setup instructions for a gitian build of BLX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BLX
...
Clone the git repositories for blx and gitian and then checkout the blx version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the blx repository
...
To build BLX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the blx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/blx/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tpowercoin/blx-core
...
For example:
```bash
URL=https://github.com/tpowercoin/blx-core.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit blx=${COMMIT} --url blx=${URL} ../BLX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit blx=${COMMIT} --url blx=${URL} ../BLX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit blx=${COMMIT} --url blx=${URL} ../BLX/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/blx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/blx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/blx-build.assert
```
